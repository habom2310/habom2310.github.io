...
var openFile = function(url) {
    ...
    reader.onload = function(readerEvent){
      ...
      image.onload = function (imageEvent) {
        ...
        var max_size = 300;// TODO : pull max size from a site config
...
$(document).on("imageResized", function (event) {
    ...
    if (event.blob && event.url) {
        ...
        $.ajax({
            url: "http://127.0.0.1:3006/predict",
            data: {"img_base64":event.url},
            dataType: 'json',
            type: 'POST',
            success: function(response){
                console.log(response);
               predict.innerHTML = "<i>I guess it is <b>" + response.data["predict"] + "</b>with <b>" + response.data["confidence"] + "</b> confidence</i>";
            }
        });
